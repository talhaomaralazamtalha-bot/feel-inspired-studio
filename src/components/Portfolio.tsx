...
export function Portfolio() {
  ...
  return <main className="portfolio-shell">
    ...
    <section id="contact" className="contact-section"><Reveal><p className="section-kicker">09 / Contact</p><h2>LET’S BUILD<br /><em>SOMETHING.</em></h2><p>I’m interested in thoughtful technology, AI, startup, and innovation projects.</p><div className="social-links"><a href="mailto:hello@example.com"><Mail /> Email</a><a href="#" aria-label="GitHub link pending"><Github /> GitHub</a><a href="#" aria-label="LinkedIn link pending"><Linkedin /> LinkedIn</a></div></Reveal><form onSubmit={e => e.preventDefault()}><label>Name<input name="name" autoComplete="name" /></label><label>Email<input name="email" type="email" autoComplete="email" /></label><label>Message<textarea name="message" rows={4} /></label><button type="submit">Send message <ArrowUpRight /></button><small>Form delivery will be connected when a preferred email endpoint is supplied.</small></form></section>
